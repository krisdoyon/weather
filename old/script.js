...
const getMoonData = async function (lat, lon) {
  const moonResponse = await fetch(
    `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${lat},${lon}/${todayISO}/?key=G2KTKKSDX2CA79SJTJAYA52LU&include=days&elements=moonrise,moonset&timezone=${meteoData.timezone}`
  );
...
const getWeatherData = async function (lat, lon) {
  ...
  try {
    await Promise.all([getMeteoData(lat, lon), getOpenData(lat, lon)]);
    setDate();
...
const displayAst = function () {
  ...
  <h3 class="heading-ast">SUN</h3>
    ...
    <div class="flex-column">
      <span class="ast-info"><strong>Rise: </strong>${convertTo12Hr(
        moonData.days[0].moonrise
      )}</span>
      <span class="ast-info"><strong>Set: </strong>${convertTo12Hr(
        moonData.days[0].moonset
      )}</span>
    </div>`;
